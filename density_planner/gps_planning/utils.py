...
def initialize_logging(args, name):
    ...
    shutil.copyfile('gps_planning/simulation_objects.py', path_log + 'simulation_objects.py')
    shutil.copyfile('gps_planning/MotionPlanner.py', path_log + 'MotionPlanner.py')
    shutil.copyfile('gps_planning/MotionPlannerGrad.py', path_log + 'MotionPlannerGrad.py')
    shutil.copyfile('gps_planning/MotionPlannerNLP.py', path_log + 'MotionPlannerNLP.py')
    shutil.copyfile('gps_planning/plan_gps_motion.py', path_log + 'plan_gps_motion.py')
...
def print_cost_increase(methods, results):
    ...
    for method in methods:
        ...
        for s in ["cost_coll", "cost_goal", "cost_bounds", "cost_uref", "sum_time"]:
            ...
            if results[method]["num_valid"] == 0:
                print("%s: %s: %.2f" % (method, s, -1))
            else:
