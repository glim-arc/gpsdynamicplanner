...
class Encoder(nn.Module):
	def __init__(self):
		...
		self.encoder = nn.Sequential(
			...
			nn.Conv3d(1, 8, 5, stride=2, padding=1),
			nn.PReLU(),
			nn.Conv3d(8, 16, 5, stride=2, padding=1),
			nn.BatchNorm3d(16),
			nn.PReLU(),
			nn.Conv3d(16, 32, 5, stride=2, padding=0),
			nn.PReLU(),

			nn.Flatten(start_dim=1),

			# linear layer
			nn.Linear(32 * 4 * 13 * 23, 10240), nn.PReLU(), nn.Linear(10240, 4096), nn.PReLU(), nn.Linear(4096, 2048),
		...
		#
		# #linear
		# self.lin = nn.Sequential(nn.Linear(32*4*13*23, 10240),nn.PReLU(),nn.Linear(10240, 4096),nn.PReLU(),nn.Linear(4096, 2048),nn.PReLU(),
...
class Decoder(nn.Module):
	def __init__(self):
		...
		self.decoder = nn.Sequential(
			...
			nn.PReLU(), nn.Linear(10240, 32*4*13*23), nn.PReLU(),

			#flatten
			nn.Unflatten(dim=1, unflattened_size=(32,4,13,23)),

			#de-convolutional layer
			nn.ConvTranspose3d(32, 16, 5,
