...
class MotionPlannerGrad(MotionPlanner):
    ...
    def optimize(self, up, epochs, initializing=False):
        ...
        if initializing:
            ...
            self.check_collisions = torch.zeros(up.shape[0], dtype=torch.bool)
    ...
    def get_cost_initialize(self, uref_traj, x_traj):
        ...
        if torch.any(self.check_bounds):
            ...
            if not torch.all(self.check_collisions) and not torch.all(self.check_gps):
                idx_check = torch.logical_and(in_bounds, torch.logical_not(self.check_collisions))
                idx_check = torch.logical_and(in_bounds, torch.logical_not(self.check_gps))
                if torch.any(idx_check):
                    self.check_collisions[idx_check] = True  # TO-DO: get the right indizes
                    self.check_gps[idx_check] = True
                    self.rms[idx_check, :, :] = 0
                    self.momentum[idx_check, :, :] = 0
                    self.counts[idx_check] = 0

        if torch.any(self.check_collisions) and torch.any(self.check_gps):
            x_check = x_traj[self.check_collisions, :, :]
            cost_coll[self.check_collisions] = self.get_cost_coll_initialize(x_check)  # for xref: 0.044s
        ...
        cost_dict = {
            ...
            "cost_coll": self.weight_gps * cost_gps,
