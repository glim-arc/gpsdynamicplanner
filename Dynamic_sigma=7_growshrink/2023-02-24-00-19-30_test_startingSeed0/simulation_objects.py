...
class DynamicObstacle(StaticObstacle):
    ...
    def forward_occupancy(self, step_size=1):
        ...
        for i in range(step_size):
            self.gps_grid[:, :, self.current_timestep + 1 + i] = self.gps_shift_array(sum,i,self.grid[:, :, self.current_timestep + i],
    ...
    def gps_shift_array(self,sum,iter, grid, step_x=0, step_y=0, fill=0):
        """
